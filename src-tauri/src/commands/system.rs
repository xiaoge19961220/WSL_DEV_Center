...
fn validate_name(name: &str) -> Result<(), String> {
    if crate::commands::wsl::list_wsl_distros()?
        .iter()
        .any(|d| d.name == name)
    {
        Ok(())
    } else {
        Err("实例不存在，请刷新列表后重试。".into())
    }
}
...
pub fn open_terminal(name: String, terminal: Option<String>) -> Result<String, String> {
    ...
    if terminal.as_deref() != Some("powershell")
        && Command::new("wt.exe")
    ...
    Command::new("powershell.exe")
        ...
        .spawn()
        .map_err(|e| format!("无法打开终端。\n位置：终端快捷入口\n退出码：不可用\n详情：{e}"))?;
    Ok(if terminal.as_deref() == Some("powershell") {
        "已请求打开 PowerShell。"
    } else {
        "Windows Terminal 不可用，已改用 PowerShell。"
    }
    .into())
}
